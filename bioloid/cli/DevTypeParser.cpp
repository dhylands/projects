...
DevTypeParser::DevTypeParser()
    : m_fs(NULL), m_devType(NULL), m_allocRegs(0), m_addDevTypeFunc(NULL) {}
...
bool DevTypeParser::ParseLine() {
    ...
    if (m_devType != NULL) {
        if (stricmp(token, "Model:") == 0) {
            if (!m_line.NextNum(&m_devType->model)) {
                LogError(
                    "Numeric model required: found '%s' %s:%d\n", m_line.PrevToken(), m_fileName,
                    m_lineNum);
                return false;
            }
        ...
        if (stricmp(token, "Register:") == 0) {
            ...
            if (m_devType->numRegs >= m_allocRegs) {
                m_allocRegs += 8;
                m_devType->reg =
                    (BLD_Reg_t*)realloc(m_devType->reg, m_allocRegs * sizeof(m_devType->reg[0]));
            }
        ...
        if (stricmp(token, "EndDeviceType") == 0) {
            LogVerbose(
                "Parsed device type: '%s' %d registers\n", m_devType->devTypeStr,
                m_devType->numRegs);
    ...
    if (stricmp(token, "DeviceType:") == 0) {
        if (m_devType != NULL) {
            LogError(
                "Duplicate DeviceType or missing EndDeviceType %s:%d\n", m_fileName, m_lineNum);
            return false;
        }
...
bool DevTypeParser::ParseRegister(BLD_Reg_t* reg) {
    ...
    if ((numBytes < 1) || (numBytes > 2)) {
        LogError(
            "Register %s: numBytes must be 1 or 2, found: %d %s:%d\n", reg->name, numBytes,
            m_fileName, m_lineNum);
        return false;
    }
    reg->flags = 0;
    ...
    if ((s = m_line.NextToken()) == NULL) {
        LogError(
            "Register %s: No mode specified (ro/rw) %s:%d\n", reg->name, m_fileName, m_lineNum);
        return false;
    }

    if (stricmp(s, "ro") == 0) {
        reg->flags |= BLD_REG_FLAG_RD;
    } else if (stricmp(s, "rw") == 0) {
        reg->flags |= BLD_REG_FLAG_RW;
    } else {
        LogError(
            "Register %s: Invalid mode specified. Expecting ro/rw, found: '%s' %s:%d\n", reg->name,
            s, m_fileName, m_lineNum);
        return false;
    }
    ...
    if (!m_line.NextNum(&minVal)) {
        if ((reg->flags & BLD_REG_FLAG_WR) != 0) {
            LogError(
                "Register %s: Min and Max required for writable registers %s:%d\n", reg->name,
                m_fileName, m_lineNum);
            return false;
        }
    ...
    if (!m_line.NextNum(&maxVal)) {
        LogError(
            "Register %s: must specify max val when using min val %s:%d\n", reg->name, m_fileName,
            m_lineNum);
        return false;
    }
    if (maxVal < minVal) {
        LogError(
            "Register %s: maxVal (%u) must be >= minVal (%u) %s:%d\n", reg->name, maxVal, minVal,
            m_fileName, m_lineNum);
        return false;
    }
    if (((reg->flags & BLD_REG_FLAG_16BIT) == 0) && (maxVal > 255)) {
        LogError(
            "Register %s: maxVal (%u) must be <= 255 for 1 byte fields %s:%d\n", reg->name, maxVal,
            m_fileName, m_lineNum);
        return false;
    }
    reg->minVal = minVal;
    ...
    if ((s = m_line.NextToken()) != NULL) {
        ...
        LogError(
            "Register %s: unrecognized field type: '%s' %s:%d\n", reg->name, s, m_fileName,
            m_lineNum);
        return false;
    }
