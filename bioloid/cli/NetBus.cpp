...
NetBus::NetBus() : m_initialized(false) {}
...
bool NetBus::Open(const char* hostStr) {
    ...
    if (!m_initialized) {
        ...
        if ((err = WSAStartup(0x0202, &wsaData)) != 0) {
            LogError(
                "Error initializing Windows Sockets: %s\n",
    ...
    if (strspn(hostName, ".0123456789") == strlen(hostName)) {
        ...
        m_sockAddr.sin_addr.S_un.S_addr = *((uint32_t*)hostent->h_addr_list[0]);
    }
    ...
    if (*portStr == '\0') {
        ...
        m_sockAddr.sin_port = htons((uint16_t)longPort);
    }
    ...
    StrPrintf(
        m_connectionInfo, sizeof(m_connectionInfo), "%s (%d.%d.%d.%d:%d)", hostStr,
        m_sockAddr.sin_addr.S_un.S_un_b.s_b1, m_sockAddr.sin_addr.S_un.S_un_b.s_b2,
        m_sockAddr.sin_addr.S_un.S_un_b.s_b3, m_sockAddr.sin_addr.S_un.S_un_b.s_b4,
